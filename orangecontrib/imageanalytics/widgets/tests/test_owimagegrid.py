...
class TestOWImageGrid(WidgetTest):
    ...
    def tests_subset_mixed_arrays(self):
        ...
        self.assertEqual(0, len(w.data_is_subset))

        # send regular subset
        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[:2])
        self.assertListEqual([True, True, False, False], w.data_is_subset)

        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[::2])
        self.assertListEqual([True, False, True, False], w.data_is_subset)

        # send mixed subset
        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[[3, 2]])
        self.assertListEqual([False, False, True, True], w.data_is_subset)

        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[[3, 0, 2]])
        self.assertListEqual([True, False, True, True], w.data_is_subset)
        ...
        # send regular subset
        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[:2])
        self.assertListEqual([True, False, False, True], w.data_is_subset)
        ...
        # send mixed subset
        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[[3, 2]])
        self.assertListEqual([False, True, True, False], w.data_is_subset)

        self.send_signal(w.Inputs.data_subset, self.fake_embeddings[[3, 0, 2]])
        self.assertListEqual([False, True, True, True], w.data_is_subset)
    ...
    def tests_subset_conditions(self):
        ...
        # reset
        self.send_signal(w.Inputs.data_subset, None)
        self.assertListEqual([], w.data_is_subset)
        ...
        self.send_signal(w.Inputs.data_subset, fake_embedding_copy)
        self.assertListEqual([True, True, True, True], w.data_is_subset)
        self.assertTrue(self.widget.Warning.incompatible_subset.is_shown())
